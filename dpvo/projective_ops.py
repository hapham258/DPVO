...
def old_iproj(disps, intrinsics, jacobian=False):
...
def old_proj(Xs, intrinsics, jacobian=False, return_depth=False):
...
def old_transform(poses, depths, intrinsics, ii, jj, jacobian=False, return_depth=False):
    """ map points from ii->jj """

    # inverse project (pinhole)
    X0, Jz = old_iproj(depths[:,ii], intrinsics[:,ii], jacobian=jacobian)
    ...
    # project (pinhole)
    x1, Jp = old_proj(X1, intrinsics[:,jj], jacobian=jacobian, return_depth=return_depth)
...
def induced_flow(poses, disps, intrinsics, ii, jj):
    ...
    coords1, valid = old_transform(poses, disps, intrinsics, ii, jj, False)
